...
def automatic_api_deprecation_detection(path1):
    ...
    def print_py_files(path,files_list):
        for file in os.listdir(path):
            ...
            #if a:
                #print('ok')
            if(os.path.isfile(os.path.join(path, str(file))) and re.search(".*\.py$",file)):
                files_list.append(os.path.join(path, str(file)))
            elif(os.path.isdir(path+str(file))):
                print_py_files(path+str(file)+"/",files_list)
        return files_list

    files_list=[]
    if (os.path.isfile(path1)) and ".py" in path1:
        files_list.append(path1)
    py_files = files_list
